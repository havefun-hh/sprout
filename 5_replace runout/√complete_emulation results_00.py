...
# """===================================================================joint======================================================================="""
# """==================================================================(1)止口======================================================================"""
# part1_radial_data_x_up = np.array(pd.read_csv("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/part1止口x.txt", sep='\t', usecols=[0,1,2,4]))
# part1_radial_data_y_up = np.array(pd.read_csv("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/part1止口y.txt", sep='\t', usecols=[0,1,2,4]))
# part2_radial_data_x_down = np.array(pd.read_csv("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/part2止口x.txt", sep='\t', usecols=[0,1,2,4]))
# part2_radial_data_y_down = np.array(pd.read_csv("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/part2止口y.txt", sep='\t', usecols=[0,1,2,4]))

# new_part1_radial_data_up = np.vstack((part1_radial_data_x_up[:, 1] + part1_radial_data_x_up[:, 3], part1_radial_data_y_up[:, 2] + part1_radial_data_y_up[:, 3])).T
# new_part2_radial_data_down = np.vstack((part2_radial_data_x_down[:, 1] + part2_radial_data_x_down[:, 3], part2_radial_data_y_down[:, 2] + part2_radial_data_y_down[:, 3])).T

# """
# np.savetxt("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/变形后part1止口坐标.csv", new_part1_radial_data_up, delimiter=',')
# np.savetxt("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/变形后part2止口坐标.csv", new_part2_radial_data, delimiter=',')
# """

# part1_circle_center_up = pre.circ(new_part1_radial_data_up)
# part2_circle_center_down = pre.circ(new_part2_radial_data_down)

# # mean_center_x = (part1_center_x_up + part2_center_x) / 2
# # mean_center_y = (part1_center_y_up + part2_center_y) / 2

# """==================================================================(2)端面====================================================================="""
# part1_flat_data_up = np.array(pd.read_csv("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/part1端面.txt", sep='\t'))
# part2_flat_data_down = np.array(pd.read_csv("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/part2端面.txt", sep='\t'))

# new_part1_flat_data_up = np.vstack((part1_flat_data_up[:, 1], part1_flat_data_up[:, 2], part1_flat_data_up[:, 3] + part1_flat_data_up[:, 4])).T
# new_part2_flat_data_down = np.vstack((part2_flat_data_down[:, 1], part2_flat_data_down[:, 2], part2_flat_data_down[:, 3] + part2_flat_data_down[:, 4])).T

# """
# np.savetxt("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/变形后part1端面坐标.csv", new_part1_flat_data_up, delimiter=',')
# np.savetxt("G:/19.12.9whole model (inp replace runout)/Step8——仿真结果/变形后part2端面坐标.csv", new_part2_flat_data_down, delimiter=',')
# """

# #svd分解法拟合法向量
# part1_normal_vector_up = pre.planefit(new_part1_flat_data_up)
# part2_normal_vector_down = pre.planefit(new_part2_flat_data_down)

# part1_center_z_up = np.mean(part1_flat_data_up[:, 3] + part1_flat_data_up[:, 4])
# part2_center_z_down = np.mean(part2_flat_data_down[:, 3] + part2_flat_data_down[:, 4])
# mean_center_z = (part1_center_z_up + part2_center_z_down) / 2

# """==================================================================变形矩阵===================================================================="""
# # A = np.array([[1, 0, rot_y, mean_center_x], [0, 1, rot_x, mean_center_y], [-rot_y ,-rot_x, 1, 0], [0, 0, 0, 1]])
# # print(A)

# results = pre.eccentric(part1_circle_center_up[1], part1_circle_center_up[2], part2_circle_center_down[1], part2_circle_center_down[2], part1_normal_vector_up[0], part1_normal_vector_up[1], part1_normal_vector_up[2], part2_normal_vector_down[0], part2_normal_vector_down[1], part2_normal_vector_down[2], mean_center_z)
# A = np.array([[1, 0, results[2], results[0]], [0, 1, results[3], results[1]], [-results[2] ,-results[3], 1, mean_center_z], [0, 0, 0, 1]])

# # m = np.dot(np.dot(p1, A), p2)

# # import matplotlib.pyplot as plt

# # n = np.linspace(1, 14280, 14280)
# # plt.plot(n,new_part1_flat_data[:, 2])
# # plt.plot(n,new_part2_flat_data[:, 2])

# end1 = time.perf_counter()
# print(f'耗时：{end1 - start1}s')
...
"""==================================================================测量位置===================================================================="""
"""==================================================================(1)止口====================================================================="""
part1_radial_data_x_down = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(0)/Step8——仿真结果/1.8_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/基准止口x.txt", sep='\t', usecols=[0,1,2,4]))
part1_radial_data_y_down = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(0)/Step8——仿真结果/1.8_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/基准止口y.txt", sep='\t', usecols=[0,1,2,4]))
part2_radial_data_x_up = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(0)/Step8——仿真结果/1.8_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/位置止口x.txt", sep='\t', usecols=[0,1,2,4]))
part2_radial_data_y_up = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(0)/Step8——仿真结果/1.8_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/位置止口y.txt", sep='\t', usecols=[0,1,2,4]))
...
"""==================================================================(2)端面===================================================================="""
part1_flat_data_down = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(0)/Step8——仿真结果/1.8_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/基准端面.txt", sep='\t'))
part2_flat_data_up = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(0)/Step8——仿真结果/1.8_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/位置端面.txt", sep='\t'))
