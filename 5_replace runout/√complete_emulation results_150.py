...
"""==================================================================测量位置===================================================================="""
"""==================================================================(1)止口====================================================================="""
part1_radial_data_x_down = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(150)/Step8——仿真结果/1.13_server_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/基准止口x.txt", sep='\t', usecols=[0,1,2,4]))
part1_radial_data_y_down = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(150)/Step8——仿真结果/1.13_server_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/基准止口y.txt", sep='\t', usecols=[0,1,2,4]))
part2_radial_data_x_up = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(150)/Step8——仿真结果/1.13_server_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/位置止口x.txt", sep='\t', usecols=[0,1,2,4]))
part2_radial_data_y_up = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(150)/Step8——仿真结果/1.13_server_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/位置止口y.txt", sep='\t', usecols=[0,1,2,4]))
...
"""==================================================================(2)端面===================================================================="""
part1_flat_data_down = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(150)/Step8——仿真结果/1.13_server_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/基准端面.txt", sep='\t'))
part2_flat_data_up = np.array(pd.read_csv("G:/19.12.25whole model (less grids)/Step5----replaced(150)/Step8——仿真结果/1.13_server_inp4圆心对准，有跳动，过盈0，大变形，加重力， 弱弹簧3300N(frictional)/位置端面.txt", sep='\t'))
