# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 15:21:06 2020
...
if __name__ == '__main__':
    ...
    n = 6
    x = [[0, 180] + [i * 10, j * 10, k * 10, l * 10] for i in range(36) for j in range(36) for k in range(36) for l in range(36)]
    PoolType = 'Thread'
    ...
    a0 = pool.map(aimFunc3, args)  # 使用pool.map函数(第一列偏心值，第二列偏心相位)
    # a = np.zeros((36, 36))
    # for i in range(36):
    #     a[i, :] = a0[i * 36:(i + 1) * 36]
...
三级：0.2 * 36 = 7s
四级：7 * 36 / 60 = 4.2min
五级：7 * 36 * 36 / (60 * 60) = 2.52h
六级：7 * 36 * 36 * 36 / (60 * 60) = 90.72h
"""
