# -*- coding: utf-8 -*-
"""
Created on Wed Mar 11 16:30:55 2020
...
if __name__ == '__main__':
    ...
    while len(res[:, 0]) - n > 0:     # 如果未固定好的级数(=总的级数-已固定好的级数)大于0，则还需继续搜索，直到一级也不剩时终止。
        ...
        #     phase = [phase[:n] + [i * 10] for i in range(36)]
        #     args = list(zip(phase, [aim] * len(phase), [n] * len(phase)))   # 返回一个迭代器，每次返回一行数据
        #     traversal = pool.map(aimFunc2, args)  # 使用pool.map函数
